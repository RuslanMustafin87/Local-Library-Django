...
class AuthorCreateTest(TestCase):
    ...
    def setUp(self):
        ...
        perms = Permission.objects.get(name='Edit author')
    ...
    def test_initial_date_of_died(self):
        login = self.client.login(username='testuser', password='12345')
        resp = self.client.get(reverse('author_create'))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_redirect_success(self):
        ...
        resp = self.client.post(reverse('author_create'), {
                'first_name': 'Petr',
                'last_name': 'Ivanov',
                'date_of_death': '10/10/2016'
            })

        print(datetime.datetime(2007, 10, 10))
